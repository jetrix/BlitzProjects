...
Global PortalCamera = CreateCamera()	

Global Portal2 = CreateSphere()		;this is the portal mesh (could be a plane etc
...
Global gate2=LoadMesh("my_stargate3.3ds")
...
FitMesh gate2,-1,-1,-1,2,2,2,True
ScaleMesh gate2,1.1,1.1,1.1

PositionEntity gate2,0,0,-100
EntityTexture gate2,g_tex
UpdateNormals gate2
...
texcam=CreateTexture(256,256,1+128+256)
EntityTexture Portal,texcam
EntityTexture Portal2,texcam
PositionEntity PortalCamera, 0, 0, -97.5
;MAIN LOOP
While Not KeyDown(1)
	...
	If campos#>2 Then 
	campos# = -97.5
    PositionEntity PortalCamera, 0, 0, 0
...
ph#=MilliSecs()/1.3        ;Frequency of waves (time)
    ...
    UpdateCubemap(texcam,PortalCamera,Portal)
    UpdateCubemap(texcam,PortalCamera,Portal2)
	Mouselook(camera)
	renderglow (0,1,1,1)
	UpdateWorld
	RenderWorld
